...
function PriceCalendarDays({ pickedMonthName, setPickedMonthName }) {
  ...
  useEffect(() => {
    const params = parseParamsFromRoute(location.pathname, location.search, query, dispatch, setRouteFrom, setRouteTo);
    // console.log('routeParams in days', params);

    async function getMonthsDaysData() {
      try {
        if (location.pathname.includes('air')) {
          const currentDate = DateTime.fromISO(query.get('start_date')).plus({ month: 1 }).toISODate();
  ...
  return (
    <section className="days">
      <PriceCalendarDaysHeader pickedMonthName={pickedMonthName} setPickedMonthName={setPickedMonthName} />
      <div className="days__inner">
        <ul className="days__week">
          {weekDays.map((day, index) => <li key={index} className="days__week-item">{day}</li>)}
        </ul>

...
function PriceCalendarMonth(props) {
  ...
  const getMonth = async evt => {
    ...
    const currentMonthDate = evt.currentTarget.querySelector('.calendar__month').dataset.date;
    ...
    if (location.pathname.includes('air')) {
      ...
      history.push({
        ...
        pathname: (location.pathname === '/price-calendar/air') ? routes.priceCalendar.airDays : routes.landing.airDays,
          ...
          <>
            {
              (betterPrice === price)
                ?
                <li
                  onClick={location.pathname.includes('air') ? getMonthDays : getMonthDaysBus}
                  className="calendar__item calendar__item--best">
                  <Link to={`/price-calendar/${transport}/days`} className="calendar__item-link">
                    <span className="calendar__month" data-date={monthDate}>{dateToMonthName(monthDate)}</span>
                    <span className="calendar__price">{price ? `от ${price.toLocaleString()} ₽` : "нет данных"}</span>

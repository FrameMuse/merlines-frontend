...
function MainSpecialCard({ bestCard, bestCardImg, cardFrom, cardTo, cardToCity, cardPrice }) {
  return (
    <li className={`special__item ${bestCard ? "special__item--big" : ""}`}>
      {
        ...
        <div className="special__item-box">
          <img className="special__item-img" src={bestCardImg} alt={cardToCity} />
        </div>
      }
      <div className="special__item-head">
        <span className="special__item-city">{cardFrom}</span>
        <Svg svgClass="special__icon-arrow" svgName="arrow" svgWidth="10" svgHeight="7" />

...
function TransportSwitcher({ isCalendar,  toSwitchTransport, setSearchTransport, searchTransportIs}) {
  ...
  const priceCalendarLocation = {
    air: (location.pathname.includes(routes.air.slice(1))) || (query.get('next') === routes.priceCalendar.air),
    train: (location.pathname.includes(routes.train.slice(1))) || (query.get('next') === routes.priceCalendar.train),
    bus: (location.pathname.includes(routes.bus.slice(1))) || (query.get('next') === routes.priceCalendar.bus)
  };
  ...
  const finMinAirPrice = (tickets) => {
    ...
    tickets?.forEach(ticket => {
      ticket.sellers.forEach(seller => {
        if(minPrice > seller.price && seller.price !== 0) minPrice = seller.price
      })
    })
    return minPrice
  }
  ...
  useEffect(() => {
    setBusMinPrice(findMinBusPrice(searchResultBusTicketData?.searchData?.tickets))
    ...
    <>
      {
        isCalendar
          ?
          <nav className={`transports ${isCalendar ? "calendar" : "ticket-list"}__transports`}>
            <Link
              className={`transports-link transports-link--plane ${(priceCalendarLocation.air) ? "transports-link--active" : ""}`}
              to={location.search ? routesData.priceCalendarSearchRoutes.air : routes.priceCalendar.air}>
              <Svg svgClass="transports-link__icon" svgName="plane" svgWidth="25" svgHeight="25" />
              <header className="transports-link__header">
                <span className="transports-link__title">{Transport.air.name}</span>
                <span className="transports-link__price">{betterPriceToString(priceCalendarData.air.betterPrice)}</span>
              </header>
            </Link>
            <Link
              className={`transports-link ${priceCalendarLocation.train ? "transports-link--active" : ""}`}
              to={routes.priceCalendar.train}>
              <Svg svgClass="transports-link__icon" svgName="train" svgWidth="25" svgHeight="25" />
              <header className="transports-link__header">
                <span className="transports-link__title">{Transport.train.name}</span>
                <span className="transports-link__price">{betterPriceToString(priceCalendarData.train.betterPrice)}</span>
              </header>
            </Link>
            <Link
              className={`transports-link ${priceCalendarLocation.bus ? "transports-link--active" : ""}`}
              to={location.search ? routesData.priceCalendarSearchRoutes.bus : routes.priceCalendar.bus}>
              <Svg svgClass="transports-link__icon" svgName="bus" svgWidth="25" svgHeight="25" />
              <header className="transports-link__header">
                <span className="transports-link__title">{Transport.bus.name}</span>
                <span className="transports-link__price">{betterPriceToString(priceCalendarData.bus.betterPrice)}</span>
              </header>
            </Link>
          </nav >
          :
          <nav className={`transports ticket-list__transports`}>
            <button name={'air'}
              ...
              <header className="transports-link__header">
                <span className="transports-link__title">{Transport.air.name}</span>
                <span className="transports-link__price">{`${betterPriceToString(airMinPrice)}`}</span>
              </header>
            </button>
            <button name={'train'}
              style={{border:"none"}}
            ...
            <button name={'bus'}
              ...
              <header className="transports-link__header">
                <span className="transports-link__title">{Transport.bus.name}</span>
                <span className="transports-link__price">{`${betterPriceToString(busMinPrice)}`}</span>
              </header>
            </button>
          </nav >
      }

...
import { selectAccessData, setIsChangedEmail } from '../../reducers/accessDataSlice';
...
function UpdAccessPopupConfirm() {
  ...
  const activate = async evt => {
    ...
    try {
      ...
      const result = await api.activation({ uid: activationCodeParams[0], token: activationCodeParams[1] });
      if (result) {
      ...
      <div className="modal__container">
        ...
        <form className="modal__form">
          <div className="input-group modal__form-group modal__form-group--margin">
            ...
            <label className="input-group__label" htmlFor="code">код из письма</label>
          </div>
          <input onClick={activate} className="modal__submit" type="submit" value="Подтвердить" />
        </form>
      </div>
    </section>
  )

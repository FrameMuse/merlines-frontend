...
const getOneTransfers = (data) =>
  data.filter((item) => item.transfers && item.transfers.length === 1)
const getTwoTransfers = (data) =>
  data.filter((item) => item.transfers && item.transfers.length === 2)
const getThreeAndMoreTransfers = (data) =>
  data.filter((item) => item.transfers && item.transfers.length > 2)

export const transfersFilter = (data, filter) => {
...
export const returnInputs = (data, { all, no, one, two, threeAndMore }) => {
  let checkboxes = []
...
export const formatDuration = (minutes) => {
  const duration = Duration.fromObject({ hours: 0, minutes: minutes })
    .normalize()
    .toObject()
...
export const shortenDate = (date) => {
  ...
  return `${dateTime.day} ${monthsShortNamesByNumbers[dateTime.month]}, ${
    weekDaysByNumbers[dateTime.weekday]
  }`
}

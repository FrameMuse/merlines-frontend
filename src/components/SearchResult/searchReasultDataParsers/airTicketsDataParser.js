...
const airTicketsDataParser = (data) => {
  let allDurations = [];
  ...
  data.forEach(sellerData => sellerData.forEach(chunk => {
    // const chunk_id = chunk.chunk_id;
    const seller_name = chunk.gates_info && chunk.gates_info[Object.keys(chunk.gates_info)[0]].label;
    const seller_id = chunk.gates_info && chunk.gates_info[Object.keys(chunk.gates_info)[0]].id;
    // const filters_boundary = chunk.filters_boundary;
    const airports = chunk.airports;
    const airlines = chunk.airlines;

    chunk.proposals && chunk.proposals.forEach(proposal => {
      ...
      const arrival_date = proposal.segment[0].flight[proposal.segment[0].flight.length - 1].arrival_date;
      const arrival_time = proposal.segment[0].flight[proposal.segment[0].flight.length - 1].arrival_time;
      const hash = departure_date + departure_time + arrival_date + arrival_time;
      const seller_price = proposal.terms[Object.keys(proposal.terms)[0]].price;
      const seller_url = proposal.terms[Object.keys(proposal.terms)[0]].url;
      const flight_baggage = proposal.terms[Object.keys(proposal.terms)[0]].flights_baggage[0][0]
      const flights_handbags = proposal.terms[Object.keys(proposal.terms)[0]].flights_handbags[0][0]
      ...
      proposal.segment.forEach(segmentItem => {
        segmentItem.flight.forEach(flightItem => {
          flightCodes.push(`${flightItem.trip_class}${flightItem.operated_by}${flightItem.number}`);
        })
      })
      ...
      const routeIndexByHash = routes.tickets.findIndex(route => route.hasOwnProperty('hash') && route.hash === hash);
...
export default airTicketsDataParser

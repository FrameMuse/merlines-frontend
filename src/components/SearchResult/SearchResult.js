...
import {selectSearchResult, setSearchData} from '../../reducers/searchResultSlice';
...
function SearchResult({ setIsOpenFilter }) {
  ...
  const loadStatuses = {success:'Success', loading:'loading', failed:'Failure' }
  ...
  useEffect(() => {  // FIXME: это при каждом изменении окна перезапускаем?) чёт такое себе...
  ...
  const getParsParamsFromRout = () => parseParamsFromRoute(location.pathname, location.search, query, dispatch, setRouteFrom, setRouteTo)
  ...
  useEffect(() => {
    if (airTicketsResult.type === loadStatuses.success) {
      console.log('Seccess')
      dispatch(setSearchData(airTicketsDataParser(airTicketsResult.result.data)));
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content={meta.generateMetaDescription(cityFrom, cityFromCode, cityTo, cityToCode, travelClass, isChildren)} />
      </Helmet>
      <ScrollOnTopTest />
      {
        airTicketsResult.type === 'Success' || (bussTicketsResult.type === 'Success' && transport === 'bus')
          ?
          <section className="ticket-list">
                  ...
                  <>
                    ...
                    <div className="form-close">
                      <button
                        ...
                        type="button">
                        <Svg svgClass="form-close__icon" svgName="arrow-angle" svgWidth="20" svgHeight="20" />
                      </button>
                    </div>
                  </>
                  :
            ...
            <div onClick={() => setIsOpenFilter(true)} className="ticket-list__open-filter">фильтры</div>
          </section>
          :
          <>
            {
              isSearchFormOpen
                ...
                <Loader loadedState={airTicketsResult.type}>
                  <SearchForm searchResult={true} />
                  <div className="form-close">
                    <button onClick={() => setIsSearchFormOpen(!isSearchFormOpen)} className="form-close__btn" type="button">
                      <Svg svgClass="form-close__icon" svgName="arrow-angle" svgWidth="20" svgHeight="20" />
                    </button>
                  </div>
                </Loader>

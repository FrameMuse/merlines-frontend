  ...
  return (
    <div className={`ticket${isOpenMoreInfo ? ' ticket--opened' : ''}`}>
        ...
        <div className="ticket__header">
          {
            carriers && carriers.map((carrier, index) =>
              <span key={index} className={`ticket__logo${(carrierLogoOne) ? '' : ' ticket__logo--rounded'}`}>
                <img src={`https://static.merlines.ru/img/air-logos/${carrierLogoSize}/${carrier.id}.png`} width={carrierLogoWidth} height={carrierLogoHeight} alt={carrier.id} />
          ...
          <div className="ticket__nav">
            <Link
              ...
              to="#">
              <Svg svgClass="ticket__notice-icon" svgName="notice" svgWidth="15" svgHeight="15" />
            ...
            <Link
              ...
              to="#">
              <Svg svgClass="ticket__star-icon" svgName="star" svgWidth="15" svgHeight="15" />
              <div className="ticket__pop-up">
                {`
                  ${!isFavouritesActive
                    ?
            ...
            <Link
              ...
              to="#">
              <Svg svgClass="ticket__share-icon" svgName="share" svgWidth="15" svgHeight="15" />
      ...
      <div className="ticket__footer">
          ...
          {voyages[0]?.flight_baggage || voyages[0]?.flights_handbags
            ?<>
              <button
                ...
                type="button"
                title={
                  'Ручная кладь ' +  toTranslateBaggageCode(voyages[0].flights_handbags).place + ' места\n' +
                  'Багаж ' +  toTranslateBaggageCode(voyages[0].flights_handbags).place + ' места'
                }
              >

                    <Svg svgClass="ticket__btn-icon" svgName="baggage" svgWidth="20" svgHeight="20" />
                бесплатно
              </button>
              <button
                className={voyages[0]?.flight_baggage && voyages[0]?.flight_baggage !== ''
                  ? "ticket__btn ticket__btn--active"
                  :"ticket__btn"}
                type="button">
                <Svg svgClass="ticket__btn-icon" svgName="baggageLg" svgWidth="40" svgHeight="20" />
        ...
        <div className="ticket__price">{separateThousand(sellersSorted[0].price)} &nbsp;₽</div>
            ...
            </div>
            {
              ...
              voyages.map((item, index) => {
                const isAirportChange = voyages?.[index + 1]
                  ? voyages?.[index + 1]?.departure_point_id !== item?.arrival_point_id
                  : false
                  return (
                    <>
                      ...
                      {transfers && transfers.length !== index ?
                        <SearchResultTicketTransfer
                          ...
                          isAirportChange={isAirportChange}/> : null}
                    </>
                  )
                }
              )

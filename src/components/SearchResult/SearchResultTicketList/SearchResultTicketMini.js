...
function SearchResultTicketMini(
  ...
  const voyageDuration = Duration.fromObject({ days: 0, hours: 0, minutes: maxDuration }).normalize().toObject();
    ...
    <div className={`ticket-mini ${isOpenAboutInfo ? "ticket-mini--opened" : ""}`}>
      ...
      <div className="ticket-mini__middle">
        <div className="ticket-mini__col">
          <time className="ticket-mini__col-item">{getSimpleTimeFromISO(departureDatetime)}</time>
          <time className="ticket-mini__col-item">{getSimpleTimeFromISO(arrivalDateTime)}</time>
        </div>
        <div className="ticket-mini__col">
          <div className="ticket-mini__col-item ticket-mini__col-item--place">
            {departurePointName}</div>
          <div className="ticket-mini__col-item ticket-mini__col-item--place">
            {arrivalPointName}</div>
        </div>
        <div className="ticket-mini__col">
          <time className="ticket-mini__col-item">{dateMountWeekday(departureDatetime)}</time>
          <time className="ticket-mini__col-item">{dateMountWeekday(arrivalDateTime)}</time>
        </div>
      </div>
        ...
        <div className="ticket-mini__bottom-item">
          {handbags
            ? handbags === ''
              ...
              : <><Svg svgClass="ticket-mini__bottom-icon" svgName="baggage" svgWidth="30" svgHeight="15" />- ручная кладь включена </>
        ...
        <div className="ticket-mini__bottom-item">
          {baggage
            ? baggage === ''
              ...
              : <><Svg svgClass="ticket-mini__bottom-icon" svgName="baggageLg" svgWidth="30" svgHeight="15" /> - багаж включён </>
      ...
      </div>
      {
        ...
        <div className="ticket-mini__info">
          <div className="ticket-mini__info-col">
            <span className="ticket-mini__info-item">Перевозчик: <b>{carrier || `Нет данных`}</b></span>
            <span className="ticket-mini__info-item">Транспорт: <b>Автобус</b></span>
            <span className="ticket-mini__info-item">Класс: <b>{`Нет данных`}</b></span>
          </div>
          <div className="ticket-mini__info-col">
            <span className="ticket-mini__info-item">Еда: <b>{`Нет данных`}</b></span>
            <span className="ticket-mini__info-item">Развлечения: <b>{`Нет данных`}</b></span>
          </div>
          <div className="ticket-mini__info-col">
            <span className="ticket-mini__info-item">Напитки: <b>{`Нет данных`}</b></span>
            <span className="ticket-mini__info-item">Зарядка: <b>{`Нет данных`}</b></span>
            <span className="ticket-mini__info-item">Wi-Fi: <b>{`Нет данных`}</b></span>
          </div>
        </div>
          :
          <div className="ticket-mini__info">

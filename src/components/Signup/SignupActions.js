...
export const signupNewUser = userData => dispatch => {
  ...
  api.registration(userData)
    ...
    .then(response => {
      ...
      console.info(`Account for ${userData.username} (${userData.email}) created successfully. Please login.`)
      dispatch({ type: CREATE_USER_SUCCESS });
      toast.success(`Спасибо за регистрацию ${response.data.first_name}${response.data.last_name && ` ${response.data.last_name}`}.`);

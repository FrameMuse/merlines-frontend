...
function HeaderNavigation() {
  ...
  useEffect(() => {
    const handleOverlayClose = evt => {
      ...
      const isClickedOnPopup = evt.target.classList.contains('nav--opened')
  ...
  return (
    <nav className={`nav ${isMenuOpened ? "nav--opened" : "nav--closed"}`}>
      <button onClick={() => setIsMenuOpened(!isMenuOpened)} className="nav__toggle" type="button">
          ...
          (windowSize.width < 992 && accessData.loginToken)
            ...
            <>
              {windowSize.width < 992
                &&
                <HeaderLink
                  href={accessData.loginToken ? routes.lk.base : `${routes.login}?next=${path}`}
                  svgClass={accessData.loginToken ? 'nav__user-on' : 'nav__user-off'}
      ...
      <div className="nav__list nav__list--setting">
        <div className="nav__item nav__item--language">
          <HeaderLink modifier="russia nav__title" href='#' svgClass="" svgName="" text="Русский язык" />
        ...
        <div className="nav__item nav__item--currency">
          <HeaderLink modifier="rub nav__title" href='#' svgClass="" svgName="" text="Российский рубль" />
          {/* <div className="nav__sublist">

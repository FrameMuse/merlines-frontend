...
const InputDepartureCity = ({getCitiesData,  setSelectedCityInSessionStorage, routToInputRef}) => {
  ...
  useEffect(() => {
    let {front: {from}} = route
    const cityNameFromRedux = from
    const cityNameFromSessionStorage = getValueFromSessionStorageByKey('cityFrontFrom')
    const cityCodeFromSessionStorage = getValueFromSessionStorageByKey('cityApiFrom')
    if(!!cityNameFromRedux) {
      setCurrentDepartureCity(cityNameFromRedux)
    } else if(!!cityNameFromSessionStorage){
      dispatch(setRouteFrom({apiRoute: cityCodeFromSessionStorage, frontRoute: cityNameFromSessionStorage}));
  ...
  const changeRoutes = () => {
    dispatch(setRouteFrom({ apiRoute: mainSearchParams.route.api.to, frontRoute: mainSearchParams.route.front.to }));
    dispatch(setRouteTo({ apiRoute: mainSearchParams.route.api.from, frontRoute: mainSearchParams.route.front.from }));
  };
  ...
  return (
    <div className="form__group form__group--departure" >
      ...
      />
      <label className="form__label" htmlFor="main-departure">откуда</label>
      <button onClick={changeRoutes} type="button" className="form__switch">Поменять местами</button>
      <DropDown

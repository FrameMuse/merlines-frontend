...
const AccordionFAQ = ({question, answer}) => {
  ...
  return (
      <div className="answers__item">
        <div className={isDisplay? "answers__item-wrap answers__item-wrap--active": "answers__item-wrap"}>
          <div className="answers__question">
            <p className="answers__question-text">{`${question}`}</p>
          </div>

          <button className={isDisplay? "answers__button answers__button--active": "answers__button"} type="button" onClick={() => setIsDisplay(!isDisplay)}>
            Ответ{' '}

...
function LkProfileEdit() {
  ...
  const editUserData = async evt => {
    ...
    if (paramsData.first_name || paramsData.last_name || paramsData.email) {
      try {
        const result = await api.editUserInfo(
        ...
        if (result) {
          ...
          if (paramsData.email) {
            ...
            toast.success('Для изменения почты необходимо повторно пройти активацию.');
          }
        }
      } catch (error) {
        if (error.response) {
      ...
      if (isSamePasswords) {
        ...
        try {
          ...
          if (error.response) {
            switch (error.response.status) {
              case 400:
                return toast.error(error.response.data.current_password[0] && 'Вы ввели неправильный текущий пароль');
              case 404:
                return toast.error(error.response.statusText ? error.response.statusText : errorMessages.error404);
              case 500:
                return toast.error(error.response.statusText ? error.response.statusText : errorMessages.error500);
  ...
  return (
    <>
      <div className="cabinet__col-wrap cabinet__col-wrap--profile">
        ...
        {isSaved && <span className="cabinet__edit-info cabinet__edit-info--active">Изменения сохранены</span>}
      </div>
      <form onSubmit={editUserData} className="form-profile">
        <div className="form-profile__row">
          <div className="input-group form-profile__group">
            ...
            <label className="input-group__label" htmlFor="name">Имя</label>
          </div>
          <div className="input-group">
            ...
            <label className="input-group__label" htmlFor="surname">Фамилия</label>
          </div>
        </div>
        {
          (currentPassword || newPassword)
            ...
            <div className="form-profile__row">
              <div className="input-group">
                ...
                <label className="input-group__label" htmlFor="email">Новый e-mail</label>
              </div>
            </div>
        }
        {
              ...
              <div className="form-profile__row">
                <div className="input-group form-profile__group">
                    ...
                    autoComplete="off" />
                  {
                    !currentPasswordValidity
                      ?
                      <label className='input-group__label input-group__label--error' htmlFor='old_password'>
                        {validationMessages.password}
                      </label>
                      :
                      <label className='input-group__label' htmlFor='old_password'>Старый пароль</label>
                  }
                  <button className={`show-password ${isCurrentPasswordHide ? 'show-password--active' : ''}`}
                    onClick={() => setIsCurrentPasswordHide(!isCurrentPasswordHide)}
                    type="button"
                ...
                <div className="input-group">
                    ...
                    autoComplete="off" />
                  {
                    !newPasswordValidity
                      ?
                      <label className='input-group__label input-group__label--error' htmlFor='new_password'>
                        {validationMessages.password}
                      </label>
                      :
                      isSamePasswords
                        ...
                        :
                        <label className='input-group__label' htmlFor='new_password'>Новый пароль</label>
                  }
                  <button className={`show-password ${isNewPasswordHide ? 'show-password--active' : ''}`}

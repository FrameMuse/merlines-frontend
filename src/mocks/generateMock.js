...
const CITIES = ['Москва', 'Париж', 'Амстердам', 'Барселона', 'Милан', 'Лондон', 'Берлин', 'Прага'];
...
const pickedMonthInfo = (month, anotherMonth) => {
  ...
  const days = anotherMonth ? new Date(anotherMonth.year, anotherMonth.monthNumber, 0).getDate() : new Date(currentYear, monthNumber, 0).getDate();
  ...
  const weekDayStart =
    anotherMonth
      ?
      (new Date(anotherMonth.year, anotherMonth.monthNumber - 1, 1).getDay() === 0 ? 7 : new Date(anotherMonth.year, anotherMonth.monthNumber - 1, 1).getDay())
      :
      (new Date(currentYear, monthNumber - 1, 1).getDay() === 0 ? 7 : new Date(currentYear, monthNumber - 1, 1).getDay());
...
const lastMonthInfo = pickedMonth => {
  const lastMonthNumber = (pickedMonth.getMonth() === 0) ? 11 : pickedMonth.getMonth() - 1;
  const lastMonthYear = lastMonthNumber === 11 ? pickedMonth.getFullYear() - 1 : pickedMonth.getFullYear();
  const lastMonthDays = new Date(lastMonthYear, lastMonthNumber + 1, 0).getDate();
  const pickedMonthWeekDayStart = new Date(pickedMonth.getFullYear(), pickedMonth.getMonth(), 1).getDay() === 0 ? 7 : new Date(pickedMonth.getFullYear(), pickedMonth.getMonth(), 1).getDay();
  const lastMonthStart = (lastMonthDays - (pickedMonthWeekDayStart - 2)) >= lastMonthDays ? 0 : (lastMonthDays - (pickedMonthWeekDayStart - 2));

  const FROM = lastMonthStart ? new Date(lastMonthYear, lastMonthNumber, lastMonthStart + 1).toISOString().slice(0, 10) : 0;
  const TO = lastMonthStart ? new Date(lastMonthYear, lastMonthNumber, lastMonthDays + 1).toISOString().slice(0, 10) : 0;
...
const nextMonthInfo = (totalDays, pickedMonth) => {
  ...
  const nextMonth = ((pickedMonth.monthNum + 1) === 13) ? 1 : (pickedMonth.monthNum + 1);
...
export { makeMonthCalendar, makeCityRoute, pickedMonthInfo, lastMonthInfo, nextMonthInfo, makeWeek };

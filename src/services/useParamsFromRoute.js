...
function useParamsFromRoute() {
  ...
  const testParamsParse = location.search && parseSearchParams(paramsForCurrentRoute);
  ...
  if (location.search) {
    ...
    const isCurrentRouteSameAsInSession = (sessionStorage.getItem('cityApiFrom') === query.get('origin'))
    ...
    if (isCityNamesInSession) {
      if (isCurrentRouteSameAsInSession) {
        ...
        dispatch(setRouteFrom({ apiRoute: testParamsParse.routeParams.origin, frontRoute: sessionStorage.getItem('cityFrontFrom') }));
        dispatch(setRouteTo({ apiRoute: testParamsParse.routeParams.destination, frontRoute: sessionStorage.getItem('cityFrontTo') }));
      } else {
        console.log('else??????');
        sessionStorage.setItem('currentController', testParamsParse.name);
        sessionStorage.setItem('mainParams', JSON.stringify(testParamsParse.routeParams));

  ...
  reducers: {
    setData: (state, action) => {
      ...
      if (months.length < 12) {
        ...
        state[transport] = { ...action.payload, betterPrice: getBetterPrice(months, transport) };
      };
    },
    ...
    setPickedMonthData: (state, action) => {
      const { days, currentMonthDate, transport, previousMonthDate, nextMonthDate, pickedMonthName } = action.payload;
      const currentMonth = days.filter(item => item.date.slice(0, 7) === currentMonthDate);
      const previousMonth = (previousMonthDate !== currentMonthDate) && days.filter(item => item.date.slice(0, 7) === previousMonthDate);
      const nextMonth = (nextMonthDate !== currentMonthDate) && days.filter(item => item.date.slice(0, 7) === nextMonthDate);
